  ...
  class Articles {
    ...
    public static function modify($id){
      ...
      if (isset($_POST['titre']) AND !empty($_POST['titre'])){
          $query = $db -> prepare('UPDATE article SET titre = :titre, description = :description, date_article = :date_article, id_user = :id_user WHERE id_article = :id');
          $query-> execute([
              'id' => $id,
              'titre' => $title,
              'description' => $descr,
              'date'=> $_POST['date_article'],
              'id_user' => 1,
          ]);
